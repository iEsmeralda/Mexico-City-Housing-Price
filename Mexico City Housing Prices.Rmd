...
```{r echo=FALSE}
Q1 <- quantile(df$surface_total_in_m2, 0.25,type = 1)  # 1st quartile
Q3 <- quantile(df$surface_total_in_m2, 0.75)  # 3rd quartile
IQR_value <- IQR(df$surface_total_in_m2)      # Interquartile range
...
``` {r echo = FALSE}
# Price per m² by borough (show only flagged)
df_flags %>%
  filter(flag_any) %>%
...
``` {r echo = FALSE}
# Total surface by property type (flagged only)
df_flags %>%
  filter(flag_any) %>%
...
# Paleta sobre log10 de USD/m² (no del precio total)
